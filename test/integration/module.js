                                ...
                                describe('start()', () => {
                                    for (const wasStartedBefore of [true, false]) {
                                        describe(`with a MediaRecorder that was${wasStartedBefore ? ' ' : ' not '}started before`, () => {
                                            ...
                                            it('should abort the encoding when removing a track', () => {
                                                const { promise, reject, resolve } = Promise.withResolvers();
                                            ...
                                            it('should encode a mediaStream as a whole', () => {
                                                const { promise, reject, resolve } = Promise.withResolvers();
                                            ...
                                            it('should encode a mediaStream as one chunk', () => {
                                                const { promise, reject, resolve } = Promise.withResolvers();
                                            ...
                                            it('should encode a mediaStream in chunks', () => {
                                                const { promise, reject, resolve } = Promise.withResolvers();
                                                const chunks = [];
                                            ...
                                            it('should encode a mediaStream with a pause', () => {
                                                const { promise, reject, resolve } = Promise.withResolvers();
                                                                ...
                                                                for (let j = bufferLength; j < audioBuffer.length; j += bufferLength) {
                                                                    ...
                                                                    if (k !== bufferLength) {
                                                                        ...
                                                                        if (numberOfExpectedZeros > 0) {
                                                                            const zeroBuffer = new Float32Array(numberOfExpectedZeros);
                                                                        ...
                                                                        for (
                                                                            ...
                                                                            compareRotatingBuffers(gain * 3 - 1, gain - 1, rotatingBuffers);
                                            ...
                                            it('should encode a mediaStream as two separate recodings', () => {
                                                const { promise, reject, resolve } = Promise.withResolvers();
                                                const blobs = [];
            ...
            describe(`with the mimeType of ${mimeType}`, () => {
                ...
                describe('constructor()', () => {
                    if (isTypeSupported) {
                        ...
                        it('should throw a NotSupportedError', () => {
                            expect(() => new MediaRecorder(mediaStream, { mimeType }))
                                .to.throw(DOMException)
                                .to.include({ code: 9, name: 'NotSupportedError' });
                        });
                    }
